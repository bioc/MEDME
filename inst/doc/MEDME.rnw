...
The test dataset contains calibration as well as experimental data for 50000 probes tiled on the human chrX. See the references for the complete dataset and additional details.
...
the \texttt{probe-level CpG content}. This depends on the genome release of the probe genomic positions. At the moment only human hg17 and 18 are possible choices.
